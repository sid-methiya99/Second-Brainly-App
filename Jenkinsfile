...
# Check backend first, as frontend depends on it
# CHANGED: Use http://localhost:3000 or a known valid path that responds quickly
                                if curl -f http://localhost:3000; then # <<<--- MODIFIED HERE (removed /api/v1/health)
...
# For more robust debugging, you could log backend container logs here
...
# Test backend's own root endpoint
                    docker exec second-brainly-backend curl -f http://localhost:3000 && echo "✅ Backend internal (self-check)" || echo "❌ Backend internal (self-check) failed" # <<<--- MODIFIED HERE
# Test frontend's own root endpoint
...
# Test frontend to backend communication (use root of backend if /api/v1/health doesn't exist)
                    docker exec second-brainly-frontend curl -f http://backend:3000 && echo "✅ Frontend to Backend communication" || echo "❌ Frontend to Backend communication failed" # <<<--- MODIFIED HERE
